...
from models import PatchGAN, ResidualGenerator
...
class ResGenTrainer(ModelTrainer):
    def __init__(self, *args):
        super(ResGenTrainer, self).__init__(*args)
        ...
        # vgg feature
        self.vgg = vgg19_bn(True).features.to(args[2])

        # turn off pretrained vgg's gradient
    ...
    def train(self, last_iteration):
        ...
        for i, datas in enumerate(self.data_loader, last_iteration):
            ...
            self.fakeB = self.generator(imageA, imageB)
    ...
    def validate(self, dataset, epoch, samples=3):
        ...
        for i, (target, style) in enumerate(zip(targets, styles)):
            ...
            fakeB = self.generator(imageA, styleB)
        ...
        save_image(result, 'resgen_val_%03d' % epoch,
    ...
    def _update_generator(self):
        ...
        loss_G_l1 = l1_loss(self.vgg(self.fakeB), self.vgg(
            self.imageB)) * self.args.lambd
